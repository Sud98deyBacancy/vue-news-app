...
<div class="fluid mx-1">
        ...
        <div class= "w-50 pr-5 p-2">
            ...
            class="form-control nav-item" 
             type="text" v-model="query.searchText" 
             placeholder="Search news items..." aria-label="Search">
          </div>
        </div>
           
  
  <h3 class="pt-3"> <span> {{ filteredArticles.length }} news reports found </span> </h3>
 ...
 computed: {
    filteredArticles(){
      if(this.query && (this.query.searchText === '' || this.query.searchText === 'all')){
        ...
        article.source.name?.includes(this.query.searchText.trim()) || 
        article.title?.includes(this.query.searchText.trim())   
        );
        return result
      }
    }
